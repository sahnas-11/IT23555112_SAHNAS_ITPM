...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            # Show the category label from column B (index 2) if available
            try:
                label_cell = ws.cell(row=row_index, column=2)
                label = str(label_cell.value).strip() if label_cell.value else singlish_input
            except Exception:
                label = singlish_input
            print(f"Testing [Row {row_index}]: {label}")
            ...
            try:
                ...
                # ── KEY FIX: if output didn't update, still write FAIL ──
                if not actual_output:
                    actual_output = prev_output  # keep whatever was there
                ...
                if expected_sinhala:
                    ...
                    status = "FAIL"  # no expected = treat as FAIL
                ...
                # ── KEY FIX: write FAIL instead of "UI Error" ──
                try:
